...
class CursoHorario(db.Model, BaseModelMixin):
    ...
    curso = db.relationship('Curso', lazy='joined', backref=db.backref('horario_curso', cascade="save-update, merge, "
                                                "delete, delete-orphan", lazy='joined') )
...
class CursoSchema(BaseSchema):
    ...
    class Meta:
        additional = ('id', 'nivel_id', 'aula_id', 'inscripcion_id', 'creador_id', 'max_alumnos', 'representacion', 'nombre', 'estado_inscripcion', 'descripcion', 'created_at')
        unknown = EXCLUDE
        ordered = True
        dump_only = ('id', )
        allow_none = ('nivel_id', 'aula_id', 'inscripcion_id','creador_id', 'max_alumnos', 'representacion', 'nombre', 'estado_inscripcion')
        required = ('nombre', 'descripcion')

class CursoModSchema(BaseSchema):
    horario = fields.Nested("HorarioSchema", many=True)
    class Meta:
        fields = ('id', 'nombre', 'horario', 'cupo', 'descripcion','representacion','estado_inscripcion')
        unknown = EXCLUDE
        ordered = True