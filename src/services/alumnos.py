...
def registrarAlumno(dni, email):
    ...
    if not alumno.email == email:
        ...
        raise UnAuthorize(
            f"El correo registrado para el dni ingresado es: {decode_email}"
        )
...
def desasociarAlumnoCurso(data):
    curso_alumno = AlumnosCursos.query.filter(
        and_(
            AlumnosCursos.curso_id == data.get("curso_id"),
            AlumnosCursos.alumno_id == data.get("alumno_id"),
        )
    ).first()
